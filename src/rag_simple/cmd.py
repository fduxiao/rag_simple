...
def cmd_build(args):
    ...
    if project is None:
        print(
            f"Unable to find a rag project. Use environ ${RAGProject.Environ} to specify."
        )
        return -1
...
def cmd_ask(args):
    ...
    if project is None:
        ...
        return -1
    return project.ask(args.question, limit=args.limit)
...
def main():
    ...
    parser_new_doc = sub_parsers.add_parser(
        "new_doc", help="make an example document file"
    )
    parser_new_doc.add_argument("path", help="document path")
    parser_new_doc.add_argument(
        "--force", "-f", action="count", help="overwrite when existing"
    )
    parser_new_doc.set_defaults(
        func=lambda x: RAGProject.new_doc(x.path, bool(x.force))
    )
    ...
    parser_ask.add_argument(
        "--limit", "-n", type=int, default=3, help="how many items you want to retrieve"
    )
    parser_ask.add_argument("question", default=None, nargs="?")
    parser_ask.set_defaults(func=cmd_ask)

    parser_retrieve = sub_parsers.add_parser(
        "retrieve", help="retrieve from vector database (chromadb)"
    )
    ...
    parser_clear.add_argument(
        "--yes", "-y", action="count", help="assume `yes` to all questions"
    )
